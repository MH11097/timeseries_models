...
class RNNModel(BaseModel):
    ...
    def train(self, train_df: pd.DataFrame, val_df: pd.DataFrame | None = None) -> dict:
        ...
        self.net = RNNNet(len(self.feature_cols), self.hidden_size, self.num_layers, self.dropout, _out_sz).to(self.device)
        ...
        print(f"Training RNN | Strategy: {self.forecast_strategy} | Device: {self.device} | Epochs: {self.epochs}, LR: {self.lr}, Hidden: {self.hidden_size}, Horizon: +{self.forecast_horizon} days, LogSales: {self.log_target}", flush=True)
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        ...
        - direct:      dự đoán tại đúng bước H.
        - multioutput: lấy output cuối (bước H) của vector H giá trị.
        - recursive:   lăn bánh H bước từ mỗi vị trí.
    ...
    def predict_all_horizons(self, df: pd.DataFrame) -> np.ndarray:
        ...
        Hàng j chứa H dự đoán từ cửa sổ kết thúc tại ngày j.
        ...
        if self.forecast_strategy == "direct":
            raise ValueError(
                ...
                "Train H model riêng hoặc đổi sang 'multioutput'/'recursive'."
            )
    ...
    def _predict_stores(self, df: pd.DataFrame, all_horizons: bool) -> np.ndarray:
        ...
        for store_id in df_pos["Store"].unique():
            mask = (df_pos["Store"] == store_id).values
            pos = np.where(mask)[0]
            store_df = df_pos.iloc[pos]
    ...
    def _pad_preds(raw: np.ndarray, n: int, offset: int) -> np.ndarray:
        full = np.zeros(n)
        if len(raw) > 0:
            full[offset:] = np.clip(raw[: n - offset], 0, None)
            full[:offset] = raw[0]
        return full
    ...
    def _run_multioutput_single(self, features: np.ndarray, H: int) -> np.ndarray:
        ...
        with torch.no_grad():
            for X, _ in loader:
                preds.append(self.net(X.to(self.device)).cpu().numpy()[:, -1])
        raw = np.concatenate(preds) if preds else np.array([])
        return self._pad_preds(raw, len(features), self.seq_len + H - 1)
    ...
    def _run_recursive(self, features: np.ndarray, sales_idx: int | None, H: int) -> np.ndarray:
        ...
        full = np.zeros(n)
        full[self.seq_len :] = np.clip(raw[: n - self.seq_len], 0, None)
        if len(raw) > 0:
            full[: self.seq_len] = raw[0]
        return full
    ...
    def _run_multioutput_all(self, features: np.ndarray, H: int) -> np.ndarray:
        ...
        with torch.no_grad():
            for X, _ in loader:
                chunks.append(self.net(X.to(self.device)).cpu().numpy())
